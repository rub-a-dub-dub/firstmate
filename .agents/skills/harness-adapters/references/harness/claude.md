...
When the project entry instead already carries an explicit decline - `hasClaudeMdExternalIncludesApproved===false` paired with `hasClaudeMdExternalIncludesWarningShown===true`, the only shape that proves the dialog actually rendered and was answered - the whole registration refuses, including the trust flag, rather than manufacture consent the human never gave, so that spawn wedges on the trust dialog before it would even reach the import one.
`hasClaudeMdExternalIncludesApproved===false` without that `hasClaudeMdExternalIncludesWarningShown===true` is not a decline but Claude Code's own pre-dialog default on a project that was never asked, and it registers exactly like an entry carrying no import flags at all.
...
A visible trust dialog means pre-registration did not take effect (or the project entry already carries an explicit decline, in the two-flag shape above) - inspect the store and the spawn's error output rather than sending keys.
