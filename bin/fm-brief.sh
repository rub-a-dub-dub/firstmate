...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   An UNKEYED one folds under a shared \`default\` bucket naming no particular decision, so it retires the moment you append any later plain unkeyed \`done:\` or \`$PAUSED_VERB:\` line - your own later status supersedes it silently, with nobody having decided anything. Key a decision that must outlive your own later phase lines.
