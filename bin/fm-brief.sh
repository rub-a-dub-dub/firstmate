...
That holds for a KEYED escalation. An UNKEYED one leaves the captain no key to close it with, so it instead retires the moment you append any later plain unkeyed \`done\` or \`$PAUSED_VERB\` line - key an escalation that must outlive your own later phase lines.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   An UNKEYED one leaves firstmate no key to close it with, so it instead retires the moment you append any later plain unkeyed \`done:\` or \`$PAUSED_VERB:\` line - key a decision that must outlive your own later phase lines.
