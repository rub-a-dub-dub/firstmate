...
A retention whose row has left this backlog entirely by the time replay runs has nothing left to reopen, so replay retires the record under its own outcome rather than the close path's stale one, which would retire it silently and leave the unreached retention unreported.
Because retiring the record discards the only durable copy of what the retention captured, that report names the recorded deliverable, or says plainly that none was recorded, so the call's disposition can still be settled with the captain; [`bootstrap-diagnostics`](../.agents/skills/bootstrap-diagnostics/SKILL.md) owns the reported line and the operator response.
...
`bin/fm-bearings-snapshot.sh` places each captain hold by its `hold_bucket`, and no wording of that hold can change the bucket it lands in or the section it is placed in.
...
The wrapper reads a hold's reason and body prose in exactly one place that changes no bucket: because a same-day filing cluster leaves Charted Next's newest-filed-first ordering unable to discriminate among its rows, a gate in the hold's own home that the hold names - textually from a live hold, or through a deferred hold's unresolved `blocked-by` - is retained past that bound rather than dropped behind an honest count.
That command's header owns the exact retention rules, the `FM_BEARINGS_GATES_PINNED` cap they are bounded by, and the `gates_retained` field a board composer reads to carry every retained row.
...
Projection regressions live in `tests/fm-fleet-snapshot-view.test.sh` (the total structured-only bucket classifier, hold-until parsing, kind-independent captain actionability, undated-hold aging, and title stripping) and `tests/fm-bearings-snapshot.test.sh` (default and expanded decision-bucket membership, deferral explanations, blocker-overflow disclosure, working-hold dual surfaces, remote-summary schema invalidation, exact leading-kind inference, artifact-kind mismatch and answered-question exclusion, kind-bearing and kindless local-only landings publishing their recorded note, scout-report precedence over competing pull-request links, and captain-hold gate retention past the Charted Next bound - same-day and excerpt-cutoff text pins, structural blocked-by pins in the main home and in a secondmate ledger, prefix and ordinary-word ids that must not pin, cross-home isolation, the live-hold-first retention cap with its disclosure, and the complete `gates_retained` field, published only by a bounded run that retained something).
