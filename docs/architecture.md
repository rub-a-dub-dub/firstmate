...
An empty rollup is never read as green on its own: when the repository has any `pull_request`(`_target`)-triggered workflow (`github_repo_has_pr_ci_workflow`), the current head must show at least one `pull_request`-event Actions run or the merge is refused, either as not-yet-arrived within a short grace window or as a suspected dropped CI event past it (`github_check_dropped_ci_event`); a repository with no such workflow, or one this read cannot confirm either way, is unaffected. This closes the gap left by a GitHub-side pull_request event that never reaches Actions for a given push, where the PR's own "Checks" summary reads identically to a repository with no CI configured at all.
