...
An *empty* rollup is the one state that reads as vacuously green there, so it is never accepted on its own; this closes the gap where the PR's own "Checks" summary reads identically for a repository with no CI configured and one whose `pull_request` event never reached Actions for this push.
...
One or more such runs hand the verdict back to the ordinary rollup logic; zero refuse the merge either way, as not-yet-arrived while the head *commit* is younger than the grace window (`FM_PR_MERGE_CI_GRACE_SECS`, 300s) and as a suspected dropped CI event once it is older.
The window is measured from the head commit's own date rather than the pull request's `updatedAt`, which any comment, label or approval bumps and which would therefore reset the clock on the ordinary approve-then-merge path.
A repository whose workflow triggers, run count, or head commit date this read cannot establish is left merging exactly as before and says so on stderr, because an inconclusive read must never become a refusal a healthy pull request cannot clear; only a positively confirmed absence refuses, and `--allow-red` does not waive it.
