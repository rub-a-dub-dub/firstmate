...
An empty rollup is never read as green on its own: when the repository has an *unfiltered* `pull_request`(`_target`)-triggered workflow (`github_repo_has_pr_ci_workflow`), the current head must show at least one Actions run for one of those armed trigger events or the merge is refused, either as not-yet-arrived within a short grace window of the pull request's last update or as a suspected dropped CI event past it (`github_check_dropped_ci_event`); a repository with no such workflow, one whose only such trigger carries a `branches`/`paths`/`types` filter (which legitimately produces no run at all for an excluded pull request), or one this read cannot confirm either way, is unaffected, and an inconclusive read prints a `note:` on stderr rather than disarming the gate in silence. This closes the gap left by a GitHub-side pull_request event that never reaches Actions for a given push, where the PR's own "Checks" summary reads identically to a repository with no CI configured at all.
