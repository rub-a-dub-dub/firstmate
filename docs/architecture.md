...
Reaching that threshold does not escalate on pane-idle time alone when the task's recorded no-mistakes run is LIVE and its own active step is reporting recent activity (`crew_run_activity_recent` in `bin/fm-classify-lib.sh`, sourced from `bin/fm-crew-state.sh`'s own `activity: recent` verdict): that authoritative recency is re-checked every time the threshold is reached, so a quiet pane during a healthy fix round is never mistaken for a wedge while a run that later goes quiet or ends still escalates on the very next crossing.
