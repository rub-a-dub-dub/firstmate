...
First it asks, once per merge attempt, whether any workflow in the repository declares a `pull_request` trigger (`github_repo_has_pr_ci_workflow`): a declared trigger *arms* the rule, while a repository that declares none genuinely has no PR CI, so absence of checks there is expected and it keeps merging exactly as it did before this gate existed.
...
One or more such runs hand the verdict back to the ordinary rollup logic; a count of zero refuses the merge unconditionally, whatever the rollup happens to show beside it, so a rollup made green by a `workflow_dispatch` diagnostic, a push-triggered run, or an external status context never satisfies this rule.
The head commit's age only picks the wording: not-yet-arrived while the commit is younger than the grace window (`FM_PR_MERGE_CI_GRACE_SECS`, 300s), and past it a message that names both a genuinely dropped event and ordinary latency, and asks for a retry before either conclusion.
That window is measured from the head commit's own date rather than the pull request's `updatedAt`, which any comment, label or approval bumps and which would therefore reset the clock on the ordinary approve-then-merge path; a commit date the read cannot establish falls back to the same past-the-window wording rather than to a merge.
Only a read taken *before* an absence is confirmed can leave the gate disarmed - an unreadable workflow listing or an unreadable run count - and it says so on stderr, because an inconclusive read must never become a refusal a healthy pull request cannot clear.
A confirmed absence always refuses and `--allow-red` does not waive it, which knowingly costs the merge of a pull request whose only matching workflow is filtered out by `branches:` or `paths:` and so produces no run at all; that exemption is tracked separately as `firstmate-pr-ci-exemption-not-pr-aware`.
