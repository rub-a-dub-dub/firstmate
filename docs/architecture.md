...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own no-mistakes run reporting recent activity and without their own task worktree being written, declared external waits and attended captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
An escalation is not raised on pane-idle time alone when the task's recorded no-mistakes run is LIVE and its own active step is reporting recent activity (`crew_run_activity_recent` in `bin/fm-classify-lib.sh`, sourced from `bin/fm-crew-state.sh`'s own `activity: recent` verdict): that authoritative recency is re-checked at every `FM_STALE_ESCALATE_SECS` crossing, so a quiet pane during a healthy fix round is never mistaken for a wedge while a run that later goes quiet or ends still escalates on the very next crossing.
Like the write deferral below, that is a deferral rather than a cancellation: the whole chain ages and re-surfaces once every `FM_PAUSE_RESURFACE_SECS`, naming the run's reported activity rather than a wedge, so a fix round that keeps logging without ever progressing cannot buy silence for the life of the run.
The recency evidence is consulted only where pane-idle time is the whole basis of the suspicion - the staleness paths - and never for the busy-pane bound below, which exists for the opposite problem and which a separate process's log activity cannot speak to; that also keeps this read off every away-mode path.
A task with no attributable run, a coarse or unreadable read, or a timed-out state read all fall straight through to the ordinary pane-based wedge timer unchanged, so this never becomes a blanket suppression.
...
The shared unkeyed `default` bucket is the one place the two readings deliberately differ: the supervisors' actionable-event map (`bin/fm-classify-lib.sh`'s `_fm_status_open_decision_origins`) retires that bucket on the crew's own later plain unkeyed `done:` line but never on a paused one, because a `paused:` line is not itself captain-relevant and so is never shown as the event that replaced the row, which would otherwise make a worker that appended `blocked:` and then paused disappear from supervision the moment the wait was declared, while OPEN DECISIONS still retires the row.
...
A check run is green when its current run is green, because GitHub leaves a cancelled run in the rollup beside the passing re-run it triggered when the base branch advanced; `bin/fm-check-rollup-lib.sh` owns that rule for both of its readers - `bin/fm-pr-merge.sh`'s merge gate and `bin/fm-bearings-snapshot.sh`'s PR-checks digest, so the summary a human or firstmate reads agrees with what the gate does - and it uses `startedAt` to clear only an older completed check run that a passing run with the same name provably replaced, while unfinished check runs and non-green status contexts are never green - the gate refuses them, and the digest separates one that is still resolving as pending from a genuine failure.
That rollup can only judge what it was told about, so a separate rule asks whether the PR's checks arrived at all; this closes the gap where the PR's own "Checks" summary reads identically for a repository with no CI configured and one whose `pull_request` event never reached Actions for this push.
The rule does not read the rollup - neither whether it is empty nor what it reports - because the rollup is the surface that collapsed those two states into one green-looking string in the first place.
The rule runs in two steps.
First it asks, once per merge attempt, whether this pull request has any `pull_request`-triggered workflow that actually applies to *it* (`github_repo_has_pr_ci_workflow`): an applicable trigger *arms* the rule, while a pull request no declared trigger covers genuinely has no PR CI coming, so absence of checks on it is expected and it keeps merging exactly as it did before this gate existed.
That question is asked per pull request rather than per repository: every workflow file is read at the pull request's own *merge* ref - the head merged into the base, the tree GitHub itself resolves a `pull_request` run from - and `github_workflow_applies_to_pr` judges the declared trigger's own `branches:` and `paths:` include filters against this pull request's base branch and changed files.
The merge ref is the only ref read, with no fallback to any other committed copy, because the merge ref's text *is* the text GitHub evaluates: a pull request that edits `.github/workflows/` is judged by its own edit, so one whose filter excludes it merges and one that widens the filter onto itself still arms, both without any separate override.
A merge ref the live view does not report leaves the read inconclusive, which disarms the rule with a note rather than arming it, exactly as an unreadable listing does.
Only an include filter confirmed *not* to cover this pull request can answer "no"; a `branches-ignore`/`paths-ignore` exclusion, a glob too complex for the heuristic to evaluate, and a changed-file list that is unreadable or capped by the API's pagination limit all count the pull request as covered, because wrongly exempting one is the unrecoverable direction while wrongly arming one only leaves it under the run count it was already under.
That stand-down is the one way this change lets a pull request merge with the gate never armed, so it names on stderr which workflow file and which filter confirmed the skip.
`pull_request_target` is deliberately not counted as arming, even though it too is a pull-request trigger: GitHub records such a run under the `pull_request_target` event and against the *base* branch's SHA, so it can never appear in the head-SHA-filtered `pull_request` count the second step performs, however that query is widened.
Absence of `pull_request` runs is therefore expected by construction in a repository whose only PR trigger is `pull_request_target` - exactly the case the first step exempts - and arming on it would instead trap every one of that repository's pull requests behind a refusal its green, running CI could never clear.
Then, for an armed repository, it counts the Actions runs at the pull request's current head whose event is `pull_request` (`github_check_dropped_ci_event`), through the API's own `?event=` filter rather than by whether any run object exists at the SHA.
That filter is the point: a `workflow_dispatch` diagnostic run leaves a run on the same SHA without carrying the `pull_request` event and without attaching to the PR, and counting it would read a manual diagnostic as proof the checks arrived - the incident that prompted this rule showed exactly that, a `total_count` of 1 from firstmate's own dispatch.
One or more such runs hand the verdict back to the ordinary rollup logic; a count of zero refuses the merge whatever the rollup happens to show beside it, so a rollup made green by a `workflow_dispatch` diagnostic, a push-triggered run, or an external status context never satisfies this rule.
While that count is still evidence, the delivery's age only picks the wording: not-yet-arrived while it is younger than the grace window (`FM_PR_MERGE_CI_GRACE_SECS`, 300s), and past it a message that names both a genuinely dropped event and ordinary latency, and asks for a retry before either conclusion.
Every age here is measured from the *later* of the head commit's own date and the pull request's `createdAt` - the moment a `pull_request` delivery could first have been expected - and never from `updatedAt`, which any comment, label or approval bumps and which would therefore reset the clock on the ordinary approve-then-merge path; a date the read cannot establish falls back to the same past-the-window wording rather than to a merge.
Taking the later of the two is what keeps retention honest: Actions retention runs from when a *run* was created, not from when a commit was written, so an old branch opened today as a fresh pull request has a months-old head commit and a minutes-old delivery whose runs are fully retained, and judging it on the commit alone would wave through the dropped delivery this rule exists to catch.
Past the retention window (`FM_PR_MERGE_CI_RETENTION_SECS`, 90 days, GitHub's own Actions run-history retention), which both the head commit and the pull request must be past, that age decides the verdict instead of the wording: the runs this step counts have been purged by then, so an empty count is evidence that expired rather than evidence CI never applied, and the gate stands down with a stderr note and leaves the merge to the ordinary rollup logic, because no retry can bring purged runs back and a refusal nothing can clear is worse than the problem.
Beyond that expiry, only a read taken *before* an absence is confirmed can leave the gate disarmed - an unestablished merge ref, an unreadable workflow listing, an unreadable run count - and every stand-down says so on stderr, because an inconclusive read must never become a refusal a healthy pull request cannot clear and an exemption must never be granted in silence.
A confirmed, unexpired absence always refuses and `--allow-red` does not waive it; what changed is which pull requests can reach that refusal at all, since one whose only matching workflow is filtered away from it by `branches:` or `paths:` is now settled as having no PR CI at the first step and merges, instead of being trapped behind a run count it could never raise.
...
A remote host's own fork-synchronization failure fails the whole run exactly as a local one does: `bin/fm-remote-secondmate-control.sh`'s `update` command raises a distinct exit status for that classifier's verdict, so the parent's sweep cannot collapse it into an ordinary transport skip.
