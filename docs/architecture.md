...
That question is asked per pull request rather than per repository: every workflow file is read at the pull request's own base ref, the nearest committed copy of what GitHub resolves a `pull_request` run from, and `github_workflow_applies_to_pr` judges the declared trigger's own `branches:` and `paths:` include filters against this pull request's base branch and changed files.
Only an include filter confirmed *not* to cover this pull request can answer "no"; a `branches-ignore`/`paths-ignore` exclusion, a glob too complex for the heuristic to evaluate, a changed-file list that is unreadable or capped by the API's pagination limit, and a pull request that edits `.github/workflows/` itself - where the base ref's committed filters are not the merged text GitHub actually ran - all count the pull request as covered, because wrongly exempting one is the unrecoverable direction while wrongly arming one only leaves it under the run count it was already under.
...
While that count is still evidence, the head commit's age only picks the wording: not-yet-arrived while the commit is younger than the grace window (`FM_PR_MERGE_CI_GRACE_SECS`, 300s), and past it a message that names both a genuinely dropped event and ordinary latency, and asks for a retry before either conclusion.
...
Past the retention window (`FM_PR_MERGE_CI_RETENTION_SECS`, 90 days, GitHub's own Actions run-history retention read off that same head-commit date) the age decides the verdict instead of the wording: the runs this step counts have been purged by then, so an empty count is evidence that expired rather than evidence CI never applied, and the gate stands down with a stderr note and leaves the merge to the ordinary rollup logic, because no retry can bring purged runs back and a refusal nothing can clear is worse than the problem.
Beyond that expiry, only a read taken *before* an absence is confirmed can leave the gate disarmed - an unreadable workflow listing or an unreadable run count - and every stand-down says so on stderr, because an inconclusive read must never become a refusal a healthy pull request cannot clear.
