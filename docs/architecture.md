...
An *empty* rollup is the one state that reads as vacuously green there, and it is never accepted on its own once the pull request's own `updatedAt` is a grace window (`FM_PR_MERGE_CI_GRACE_SECS_DEFAULT`, 300s) or more old: zero checks reported that long after the last push is not delivery latency, so the merge is refused.
Inside that window an empty rollup behaves exactly as it always has, because refusing a pull request in the first minutes after a push would false-alarm on every fresh push.
A rollup that reported anything at all is untouched by this rule at any age, and a pull request that is not all three of open, non-draft and mergeable-clean is not judged by it at all: a conflict legitimately produces no `pull_request` run, and so does a draft in a repository that skips CI on drafts, while the `mergeable`/`DIRTY` and draft refusals already name either one with the remedy that applies.
The rule reads only the live `gh pr view` JSON already fetched for the other pre-merge conditions and adds no API call; it deliberately asks no workflow file whether the repository ought to have CI, because this repository's own `ci.yml` declared, for its entire history, a valid `pull_request` trigger whose only filter is the base branch these pull requests already target, while GitHub dropped every delivery, so a workflow-file prediction would have been satisfied by exactly the absence it exists to detect.
This closes the gap where the PR's own "Checks" summary reads identically to a repository with no CI configured at all.
Three consequences of dropping that workflow-file distinction are intentional.
Absence of any check is no longer distinguishable from a dropped one, so a repository that configures no PR CI at all also refuses here once one of its pull requests sits past the window with an empty rollup, unwaivably; such a merge has to happen by a path this script does not gate, which is the accepted price of never again reading "no checks configured" as green.
The rule keys on the rollup being *empty* rather than on whether a `pull_request` event was delivered, so a check attached to the same head by any other trigger - a push-triggered workflow, a scheduled run, a `workflow_dispatch` leaving a visible check - makes the rollup non-empty and the rule does not fire even when the `pull_request` delivery was what was dropped.
And `updatedAt` is bumped by any pull request activity, not only a push, so a stale dropped-event pull request commented on or approved shortly before a merge attempt reads as fresh and skips the refusal.
