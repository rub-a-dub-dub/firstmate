...
A suite slow enough that a caller's own invocation budget can cut its run off mid-stream declares how many results it will report with `plan` from `tests/lib.sh`, so a stream that stops early is mechanically distinguishable from a complete one instead of reading like a failure.
Derive that declared count from the list of tests the script runs rather than hand-counting it, so adding a test cannot leave the declaration behind.
